...
import { handleClientMessage } from "./messageDispatcher.js";
import { getOrCreateSession, removeClientFromSession } from "./sessionManager.js";
  ...
  client.on("message", async (raw) => {
    try {
      ...
      if (client.readyState === WebSocket.OPEN) {
        client.send(JSON.stringify({ error: detail, type: "sync_error" }));
      }
    }
  });
}
