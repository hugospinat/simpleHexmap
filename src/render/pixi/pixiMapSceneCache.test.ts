...
describe("pixi map scene cache", () => {
  it("builds level scenes lazily", () => {
    ...
    expect(scene.cellsByHex.get(hexKey({ q: 0, r: 0 }))?.cell.type).toBe(
      "plain",
    );
  });
  ...
  it("patches cached source-level cells after operations", () => {
    ...
    const dirtySet = cache.applyOperations(nextWorld, [
      {
        type: "set_tiles",
        tiles: [{ hidden: false, q: 0, r: 0, terrain: "forest" }],
      },
    ]);
    ...
    expect(
      cache.getLevelScene(3, nextWorld).cellsByHex.get(hexKey({ q: 0, r: 0 }))
        ?.cell.type,
    ).toBe("forest");
  });
  ...
  it("materializes feature records for renderer layers", () => {
    ...
    expect(
      cache.getLevelScene(3, world).cellsByHex.get(hexKey({ q: 0, r: 0 }))
        ?.feature?.kind,
    ).toBe("city");
  });
});
