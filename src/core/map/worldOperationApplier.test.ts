...
} from "./worldOperationApplier";

function applyOperationsSequentially(
...
describe("world operation applier", () => {
  it("applies terrain batches with the same result as sequential operation application", () => {
    ...
    const operations: MapOperation[] = [
      ...
      {
        type: "set_tiles",
        tiles: [{ q: 1, r: 0, terrain: "water", hidden: true }],
      },
      {
        type: "set_tiles",
        tiles: [{ q: 2, r: 0, terrain: "hill", hidden: false }],
      },
      {
        type: "set_tiles",
        tiles: [{ q: 1, r: 0, terrain: null, hidden: false }],
      },
    ];

    expectSameMapData(
      applyOperationsToWorld(world, operations),
      applyOperationsSequentially(world, operations),
    );
  });
  ...
  it("applies faction territory batches with the same result as sequential operation application", () => {
    ...
    const operations: MapOperation[] = [
      {
        ...
        territories: [{ q: 0, r: 0, factionId: "red" }],
      },
      {
        type: "set_faction_territories",
        territories: [{ q: 1, r: 0, factionId: "red" }],
      },
      {
        type: "set_faction_territories",
        territories: [{ q: 1, r: 0, factionId: null }],
      },
    ];
    ...
    expect(
      getFactionLevelMap(batched, SOURCE_LEVEL).get(hexKey({ q: 0, r: 0 })),
    ).toBe("red");
    expect(
      getFactionLevelMap(batched, SOURCE_LEVEL).has(hexKey({ q: 1, r: 0 })),
    ).toBe(false);
  });
});
