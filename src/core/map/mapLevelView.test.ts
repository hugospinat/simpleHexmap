...
import { buildMapLevelView } from "./mapLevelView";
...
describe("world view selectors", () => {
  it("memoizes derived data by world identity and level", () => {
    ...
    expect(buildMapLevelView(world, 3)).toBe(buildMapLevelView(world, 3));
    expect(buildMapLevelView(world, 2)).not.toBe(buildMapLevelView(world, 3));
  });
  ...
  it("collects render-facing maps in one derived view", () => {
    ...
    const view = buildMapLevelView(world, 3);
