...
function getStoredFactionMap(world: MapState): FactionMap {
...
function sourceHasTile(world: MapState, key: string): boolean {
  return Boolean(world.levels[SOURCE_LEVEL]?.has(key));
}

function sanitizeSourceAssignments(world: MapState): FactionLevelMap {
...
export function addFaction(world: MapState, faction: Faction): MapState {
...
export function removeFaction(world: MapState, factionId: string): MapState {
  const factions = getStoredFactionMap(world);
...
function setSourceFactionAssignment(
  ...
  if (factionId && !factions.has(factionId)) {
    return { changed: false, mapState: world };
  }
