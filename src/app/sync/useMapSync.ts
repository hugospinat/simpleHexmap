...
import { applyMapOperationToWorld, type MapOperation } from "@/app/io/mapOperations";
...
type UseMapSyncOptions = {
  ...
  mapId: string;
  resetWorldFromCurrent: (deriveNextState: (currentState: World) => World) => void;
};
...
export function useMapSync({ clearPreviewWorld, mapId, resetWorldFromCurrent }: UseMapSyncOptions) {
  ...
  const sessionRef = useRef(createClientMapSession(clientIdRef.current));
  ...
  useEffect(() => {
    ...
    return () => {
      disposed = true;
      clearReconnectTimer();
      clearClientMapSession(sessionRef.current);
