...
import { applyMapOperationToWorld } from "@/app/io/mapOperations";
import {
...
describe("map edit commands", () => {
  it("paints terrain by emitting source-level set_tile operations", () => {
    ...
    expect(serializeWorld(applyCommandOperations(world, result.operations))).toEqual(serializeWorld(result.world));
  });

  it("executes business-shaped map commands through one entrypoint", () => {
    const result = executeMapCommand(createEmptyWorld(), {
      type: "paintTerrain",
      level: 3,
      axial: { q: 0, r: 0 },
      terrainType: "plain"
    ...
    expect(getLevelMap(result.world, 3).get("0,0")?.type).toBe("plain");
  });

  it("erases terrain with explicit delete operations", () => {
    const painted = commandPaintTerrain(createEmptyWorld(), 2, { q: 0, r: 0 }, "forest").world;
    ...
    expect(getLevelMap(result.world, 3).size).toBe(0);
  });
  ...
  it("emits domain-shaped road operations instead of record diffs", () => {
    ...
    const removed = commandRemoveRoadConnectionsAt(added.world, 3, { q: 0, r: 0 });
    ...
    expect(getRoadLevelMap(removed.world, 3).size).toBe(0);
  });
