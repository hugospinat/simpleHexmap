...
import { findMapTokenProfileAtPoint } from "./mapTokenHitTest";

describe("map token hit testing", () => {
  it("finds a visible token under the pointer", () => {
    const paintedWorld = addTile(createEmptyWorld(), 3, { q: 0, r: 0 }, "plain");
    const world = setCellHidden(paintedWorld, 3, { q: 0, r: 0 }, false);

    expect(findMapTokenProfileAtPoint({
      center: { q: 0, r: 0 },
      level: 3,
      point: { x: 250, y: 250 },
      tokens: [{ color: "#ff0000", profileId: "profile-a", q: 0, r: 0 }],
