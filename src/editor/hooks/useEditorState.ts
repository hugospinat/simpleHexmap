...
import { useMapSync } from "@/app/sync/useMapSync";
...
import {
  ...
  takeRedoOperations,
  takeUndoOperations
} from "@/editor/history/mapOperationHistory";
...
type ActiveEditorGesture =
  | { kind: "terrain"; gesture: EditGesture; worldBefore: World }
  | { kind: "faction"; gesture: FactionGesture; worldBefore: World }
  | { kind: "river"; gesture: RiverGesture; worldBefore: World }
  | { kind: "road"; gesture: RoadGesture; worldBefore: World }
  | { kind: "fog"; gesture: FogGesture; worldBefore: World };
...
export function useEditorState({ initialWorld, mapId, role }: UseEditorStateOptions) {
  ...
  const { resetFromCurrent, world: authoritativeWorld } = useAuthoritativeWorld<World>(() => initialWorld);
  const { changeLevelByDelta, changeVisualZoom, setCenter, view, visualZoom } = useCamera();
  const [gesturePreviewWorld, setGesturePreviewWorld] = useState<World | null>(null);
  ...
  const { sendOperations, syncStatus } = useMapSync({
  ...
  const submitLocalOperations = useCallback(
    (operations: MapOperation[], worldBefore: World = authoritativeWorld) => {
